...
export default function DownloadDetection() {
  ...
  return (
    <section className="react-download-detection" aria-labelledby="download-detection-title">
      ...
      <button type="button" disabled={isChanging} aria-pressed={enabled} onClick={handleToggle}>
        {enabled ? t("downloadDetection.disable") : t("downloadDetection.enable")}
      </button>
      {permissionDenied && <p role="alert">{t("downloadDetection.denied")}</p>}
      {recent && (
        <p className="react-download-detection__recent" role="status">
          {t("downloadDetection.recent", { fileName: recent.fileName })}
        </p>
      )}
    </section>
  );
