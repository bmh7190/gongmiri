...
공미리의 기본 기능은 `storage` 권한만 사용한다. 다운로드 감지는 사용자가 뷰어에서 기능을 켤 때 요청하는 `downloads` 선택 권한으로 제공한다. 완료된 ZIP 후보가 있으면 확장 아이콘에 `ZIP` 배지를 표시하고, 아이콘을 누르면 기존 전체 화면 뷰어를 연다. 파일 내용은 자동으로 읽지 않으며 사용자가 뷰어에서 해당 파일을 선택한다.
